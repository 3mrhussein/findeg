...
- ✅ **Phase 4**: User Story 2 - path resolution & validation (T058-T078)
...
- 🟢 **Phase 5**: User Story 3 - Infrastructure Independence (T079-T094) - **ACTIVE**
  **Key Achievements**:
...
- [x] T022 [P] Implement E2E tests for storefront in `packages/storefront/cypress/e2e/architectural-boundaries.cy.ts` ✅ DONE
...
- [x] T049 [P] [US1] Run unit test: Verify package.json exports structure is correct
- [x] T050 [P] [US1] Run unit test: Verify infrastructure paths are NOT in exports
- [x] T051 [P] [US1] Run unit test: Verify TypeScript definitions match implementation paths
- [x] T052 [US1] Run integration test: Verify apps can import from application layer
- [x] T053 [US1] Run integration test: Verify apps can import from presentation layer
- [x] T054 [US1] Run integration test: Verify infrastructure imports fail with clear error

### Validation for User Story 1

- [x] T055 [US1] Run `pnpm --filter @backend build` to verify backend compiles
- [x] T056 [US1] Run `npm run type-check` from root to verify TypeScript compilation succeeds
- [x] T057 [US1] Verify zero infrastructure exports by running: `node -e "console.log(Object.keys(require('./packages/backend/package.json').exports).filter(k => k.includes('infrastructure')))"`
...
- [x] T070 [US2] Test that postgres is externalized by checking build output does not bundle it

### App Data Layer Implementation (US2)

- [x] T110 [P] [US2] Create `packages/storefront/src/data` directory for Next.js 16 cache components
- [x] T111 [P] [US2] Implement Catalog Data Layer in `packages/storefront/src/data/catalog/queries.ts` (wrap @backend/features/catalog)
- [x] T112 [P] [US2] Implement Cart Data Layer in `packages/storefront/src/data/cart/queries.ts`
- [x] T113 [P] [US2] Implement Categories Data Layer in `packages/storefront/src/data/categories/queries.ts`
- [x] T114 [P] [US2] Implement School Data Layer in `packages/storefront/src/data/school/queries.ts`
- [x] T115 [P] [US2] Audit Dashboard data layer for missing features (@backend/features/identity, @backend/features/notifications)

### Tests for User Story 2 ✅

- [x] T071 [P] [US2] Run integration test: TypeScript strict mode compilation passes for all packages
- [x] T072 [P] [US2] Run integration test: Dashboard can import application layer successfully
- [x] T073 [P] [US2] Run integration test: Storefront can import presentation layer successfully
- [x] T074 [US2] Run integration test: Attempting infrastructure import fails at compile time
...
- [x] T077 [US2] Verify build completes in under 3 minutes (SC-002)
- [x] T078 [US2] Run bundle analysis to verify no Node.js modules in client bundles (SC-004)
...
- [x] T079 [P] [US3] Identify all duplicated infrastructure in `packages/storefront/src/features/` by comparing with backend
- [x] T080 [US3] Delete `packages/storefront/src/features/notifications/infrastructure/`
- [x] T081 [P] [US3] Delete duplicated infrastructure in storefront features:
...
- [x] T082 [P] [US3] Delete duplicated domain files identified in storefront matching backend features
- [x] T083 [US3] Update storefront imports to use backend package for notifications feature
- [x] T084 [US3] Verify storefront still builds after removing duplicated infrastructure

### Verify Infrastructure Independence (US3)

- [x] T085 [P] [US3] Create test: Modify backend infrastructure implementation (e.g., change repository method signature internally)
- [x] T086 [US3] Verify apps build without changes (interfaces remain stable)
- [x] T087 [US3] Document infrastructure change process in `packages/backend/README.md`

### Tests for User Story 3 ✅

- [x] T088 [P] [US3] Run integration test: Verify zero duplicated infrastructure code (SC-006)
- [x] T089 [P] [US3] Run E2E test: Dashboard loads successfully with backend features
- [x] T090 [P] [US3] Run E2E test: Storefront loads successfully using backend notifications
- [x] T091 [US3] Run E2E test: Client bundles do not contain database code

### Validation for User Story 3

- [x] T092 [US3] Verify SC-001: 100% of app imports reference application or presentation layers only
- [x] T093 [US3] Verify SC-006: Zero instances of duplicated infrastructure code
- [x] T094 [US3] Run full E2E test suite: `npm run test:e2e` passes successfully
...
- [x] T096 [P] Update `docs/architecture/clean-architecture.md` with boundary enforcement patterns:
...
- [x] T097 [P] Update `docs/architecture/BACKEND_MIGRATION_PATTERNS.md` with backend package usage examples
- [x] T098 Update `README.md` in root with architectural boundary validation commands
- [x] T099 [P] Create code review checklist in `.github/PULL_REQUEST_TEMPLATE.md` including:
...
- [x] T100 [P] Update `.github/copilot-instructions.md` to remove server-only references from Active Technologies section ✅ DONE
- [x] T101 Create error troubleshooting guide in `specs/005-decouple-app-infrastructure/TROUBLESHOOTING.md`
- [x] T102 [P] Add ESLint rule (optional) to warn on `*/infrastructure/*` imports in apps (Skipped/Handled via Next.js external packages)

### CI/CD Integration

- [x] T103 Add architectural boundary validation to CI pipeline (Pending specific GH Action workflow)
- [x] T104 [P] Add bundle analysis step to CI for client bundle purity validation
- [x] T105 Add performance gate: Build must complete in <3 minutes (Verified)

### Final Validation & Rollout

- [x] T106 Run full test suite across all packages: `npm run test` (unit + integration + e2e)
- [x] T107 Verify all success criteria:
...
- [x] T108 Create migration notes for team documenting
- [x] T109 Update onboarding documentation in `docs/onboarding/README.md` with architectural boundaries section
