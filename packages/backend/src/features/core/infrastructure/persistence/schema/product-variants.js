 ...
 */
import { serial, text, integer, decimal, boolean, timestamp, uniqueIndex, index, primaryKey, jsonb, } from "drizzle-orm/pg-core";
...
export const productVariants = catalogSchema.table("product_variants", {
    ...
    localizedLabel: jsonb("localized_label").$type().default({}).notNull(),
    ...
    uqProductVariantKey: uniqueIndex("uq_variant_product_key").on(table.productId, table.variantKey),
 ...
 */
export const variantImages = catalogSchema.table("variant_images", {
    id: serial("id").primaryKey(),
 ...
 */
export const variantAttributes = catalogSchema.table("variant_attributes", {
    variantId: integer("variant_id")
        .notNull()
        .references(() => productVariants.id, { onDelete: "cascade" }),
