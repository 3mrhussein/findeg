 ...
 */
import { serial, integer, text, decimal, boolean, timestamp, uniqueIndex, index, jsonb, } from "drizzle-orm/pg-core";
 ...
 */
export const variantSellableUoms = catalogSchema.table("variant_sellable_uoms", {
    ...
    uqVariantUom: uniqueIndex("uq_variant_sellable_uoms_variant_uom").on(table.variantId, table.uomCode),
    idxVariant: index("idx_variant_sellable_uoms_variant").on(table.variantId),
}));
/**
 * Variant Price Lists
 */
export const variantPriceLists = catalogSchema.table("variant_price_lists", {
    id: serial("id").primaryKey(),
    /** FK to the specific variant (SKU) */
    variantId: integer("variant_id")
        .notNull()
    ...
    customerGroup: text("customer_group").$type().notNull(),
    /** Which unit of measure this price applies to */
    uomCode: text("uom_code").$type().notNull(),
    /** Default currency */
    currency: text("currency").$type().notNull().default("EGP"),
    ...
    uqVariantPriceList: uniqueIndex("uq_variant_price_lists_variant_group_uom").on(table.variantId, table.customerGroup, table.uomCode, table.minQty),
