import { serial, integer, text, timestamp, varchar, boolean, primaryKey, uniqueIndex, index, } from "drizzle-orm/pg-core";
 ...
 */
export const authAccounts = identitySchema.table("auth_accounts", {
    id: serial("id").primaryKey(),
    userId: integer("user_id")
        .notNull()
        .references(() => users.id, { onDelete: "cascade" }),
    provider: varchar("provider", { length: 40 }).$type().notNull(),
    providerAccountId: varchar("provider_account_id", { length: 255 }).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
    uqProviderAccount: uniqueIndex("uq_auth_accounts_provider_account").on(table.provider, table.providerAccountId),
...
export const permissions = identitySchema.table("permissions", {
    id: serial("id").primaryKey(),
    code: varchar("code", { length: 160 }).$type().notNull().unique(),
    name: text("name").notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
...
export const userRoles = identitySchema.table("user_roles", {
    ...
    roleId: integer("role_id")
        .notNull()
        .references(() => roles.id, { onDelete: "cascade" }),
    scope: varchar("scope", { length: 20 }).$type().notNull().default("global"),
    organizationId: integer("organization_id").references(() => organizations.id, {
        onDelete: "cascade",
    }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => ({
    uqUserRoleScope: uniqueIndex("uq_user_roles_user_role_scope_org").on(table.userId, table.roleId, table.scope, table.organizationId),
    idxUserRolesUser: index("idx_user_roles_user_id").on(table.userId),
}));
/**
 * Per-user permission overrides.
 */
export const userPermissions = identitySchema.table("user_permissions", {
    id: serial("id").primaryKey(),
    userId: integer("user_id")
        .notNull()
        .references(() => users.id, { onDelete: "cascade" }),
    ...
    uqUserPermission: uniqueIndex("uq_user_permissions_user_permission").on(table.userId, table.permissionId),
...
export const organizationMemberships = identitySchema.table("organization_memberships", {
    ...
    uqOrganizationMembership: uniqueIndex("uq_organization_memberships_org_user").on(table.organizationId, table.userId),
    idxOrganizationMembershipUser: index("idx_organization_memberships_user_id").on(table.userId),
}));
/**
 * Tokenized saved payment methods.
 */
export const paymentMethods = identitySchema.table("payment_methods", {
    id: serial("id").primaryKey(),
    userId: integer("user_id")
        .notNull()
        .references(() => users.id, { onDelete: "cascade" }),
 ...
 */
export const guestPrincipals = identitySchema.table("guest_principals", {
    id: serial("id").primaryKey(),
