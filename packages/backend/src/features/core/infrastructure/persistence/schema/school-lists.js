 ...
 */
import { serial, text, integer, boolean, timestamp, uniqueIndex, index, jsonb, } from "drizzle-orm/pg-core";
 ...
 * school_lists Table
 */
export const schoolLists = schoolEngineSchema.table("school_lists", {
    ...
    /** Localized content */
    localizedTitle: jsonb("localized_title").$type().notNull(),
    ...
    idxSchoolListsCode: index("idx_school_lists_code").on(table.accessCode),
    uqSchoolYearGrade: uniqueIndex("uq_school_year_grade").on(table.slug, table.academicYear, table.grade),
}));
/**
...
export const schoolListItems = schoolEngineSchema.table("school_list_items", {
    ...
    localizedLabel: jsonb("localized_label").$type().notNull(),
    localizedNote: jsonb("localized_note").$type(),
 ...
 */
export const schoolListItemAlternatives = schoolEngineSchema.table("school_list_item_alternatives", {
    id: serial("id").primaryKey(),
...
export const schoolListItemAlternativesRelations = relations(schoolListItemAlternatives, ({ one }) => ({
