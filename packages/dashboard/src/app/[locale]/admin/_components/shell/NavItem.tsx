...
export function NavItem({ item, collapsed = false, locale = "en", depth = 0 }: NavItemProps) {
  ...
  // or if it's the root of the current path (except for /admin which is dashboard)
  const isActive =
    item.href === "/admin"
  ...
  if (item.portalRoles && !isSystemAdmin) {
    if (!portalRole || !item.portalRoles.includes(portalRole as any)) {
      return null;
    }
  }
  ...
  const rowContent = (
    ...
    >
      ...
      {!collapsed && (
        <>
          <span
            className={cn("ml-[10px] truncate flex-1", depth > 0 ? "text-[12px]" : "text-[14px]")}
          >
            {label}
          </span>
