...
async function filesUnder(root, directory) {
  ...
  const files = await Promise.all(entries.map(async (entry) => {
    ...
    if (entry.isDirectory()) return ignoredDirectories.has(entry.name) ? [] : filesUnder(root, relativePath);
    return entry.isFile() && sourceFile.test(entry.name) ? [relativePath] : [];
  }));
  return files.flat();
}

function prohibitedDependencies(relativePath, contents) {
  const imports = [...contents.matchAll(/(?:\b(?:import|export)\s+(?:[^"']*?\s+from\s+)?|\bimport\s*\()\s*["']([^"']+)["']/g)]
    .map((match) => match[1]);
  return imports.flatMap((imported) => {
    ...
    if (relativePath.startsWith("frontend/") && /@findeg\/backend(?:\/.*)?\/infrastructure(?:\/|$)/.test(imported)) {
      return [`Prohibited dependency: ${relativePath} imports ${imported}`];
    }
    if (relativePath.startsWith("backend/src/features/") && (imported === "next" || imported.startsWith("next/"))) {
      return [`Prohibited dependency: ${relativePath} imports ${imported}`];
    }
...
export async function runArchitectureCheck(root = process.cwd()) {
  ...
  for (const document of canonicalDocuments) {
    const exists = await readFile(resolve(root, document)).then(() => true).catch(() => false);
    if (!exists) missingDocuments.push(`Missing canonical documentation: ${document}`);
  }

  const sourceFiles = (await Promise.all(sourceRoots.map((directory) => filesUnder(root, directory)))).flat();
  const violations = (await Promise.all(sourceFiles.map(async (relativePath) => (
    prohibitedDependencies(relativePath, await readFile(resolve(root, relativePath), "utf8"))
  )))).flat();

  return [...missingDocuments, ...violations.sort()];
}
