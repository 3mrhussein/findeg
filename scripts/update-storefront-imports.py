#!/usr/bin/env python3
"""
Update component imports in storefront package to use @findeg/ui
"""
...
# Base directory for storefront package
STOREFRONT_SRC = Path(__file__).parent.parent / "packages" / "storefront" / "src"
...
def main():
    ...
    if not STOREFRONT_SRC.exists():
        print(f"Error: Storefront src directory not found: {STOREFRONT_SRC}")
        return 1
    ...
    for file_path in STOREFRONT_SRC.rglob("*.ts*"):
        ...
        if update_file_imports(file_path):
            files_modified += 1
            print(f"Updated: {file_path.relative_to(STOREFRONT_SRC.parent)}")
