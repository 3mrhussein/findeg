...
export async function checkTagSlugAvailableRaw(slug: string, excludeId?: number): Promise<boolean> {
  const where = excludeId ? and(eq(tags.slug, slug), ne(tags.id, excludeId)) : eq(tags.slug, slug);
