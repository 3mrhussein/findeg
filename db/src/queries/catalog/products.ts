...
export async function getByCategory(categoryId: number): Promise<ProductRow[]> {
  return db.select().from(products).where(eq(products.categoryId, categoryId)).limit(50);
}
...
export async function getByBrand(brandId: number): Promise<ProductRow[]> {
  return db.select().from(products).where(eq(products.brandId, brandId)).limit(50);
}
 ...
 */
export async function getByIdWithBrand(
  id: number,
): Promise<{ product: ProductRow; brand: BrandRow | null } | null> {
  const results = await db
