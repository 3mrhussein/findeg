/**
 * Query Primitives for Admin Search Analytics
 *
 * Pure database queries for search log analysis.
...
function getStartDate(days?: number): Date | null {
  if (!days) return null;
  const date = new Date();
...
export async function getMetrics(days?: number): Promise<SearchAnalyticsMetrics> {
...
export async function getTopSearches(days?: number, limit: number = 50): Promise<TopSearchQuery[]> {
  const startDate = getStartDate(days);
  const whereClause = startDate ? gte(searchLogs.createdAt, startDate) : undefined;
...
export async function getLanguageBreakdown(days?: number): Promise<LanguageBreakdown[]> {
  const startDate = getStartDate(days);
  const whereClause = startDate ? gte(searchLogs.createdAt, startDate) : undefined;
