...
export function RecentActivityWidget({ logs, locale = "en" }: RecentActivityWidgetProps) {
  ...
  const getActionIcon = (action: string) => {
    if (action.includes("CREATE")) return <Plus className="h-4 w-4 text-emerald-600" />;
    if (action.includes("UPDATE")) return <Edit className="h-4 w-4 text-blue-600" />;
    if (action.includes("DELETE")) return <Trash className="h-4 w-4 text-rose-600" />;
    return <Activity className="h-4 w-4 text-slate-600" />;
  };

  const getActionBg = (action: string) => {
    if (action.includes("CREATE")) return "bg-emerald-50 border-emerald-100";
    if (action.includes("UPDATE")) return "bg-blue-50 border-blue-100";
    if (action.includes("DELETE")) return "bg-rose-50 border-rose-100";
  ...
  return (
    <Card className="col-span-1 md:col-span-2 border-slate-200 shadow-sm">
      ...
      <CardContent>
        {logs.length === 0 ? (
          <div className="py-12 text-center text-sm text-slate-500">{t("NoActivity")}</div>
        ) : (
              ...
              <div key={log.id} className="flex items-start gap-4">
                ...
                <div className="flex flex-col space-y-1">
                  <p className="text-sm font-medium leading-none text-slate-800">
                    ...
                    <span className="text-slate-500 font-normal">#{log.entityId.slice(0, 8)}</span>
                  </p>
