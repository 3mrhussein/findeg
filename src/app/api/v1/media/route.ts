...
export async function POST(request: NextRequest) {
  return withAdmin(request, async () => {
    try {
      ...
      return apiError(error instanceof Error ? error.message : "Failed to upload media file", 500);
    }
  });
}
...
export async function DELETE(request: NextRequest) {
  return withAdmin(request, async () => {
    try {
      ...
      return apiError(error instanceof Error ? error.message : "Failed to delete media file", 500);
    }
  });
}
